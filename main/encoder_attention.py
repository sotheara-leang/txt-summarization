...
class EncoderLinearAttention(nn.Module):

    def __init__(self):
        super(EncoderAttention, self).__init__()

        self.w_e = nn.Linear(2 * conf.get('enc-hidden-size'), conf.get('dec-hidden-size'), False)
        self.w_d = nn.Linear(conf.get('dec-hidden-size'), conf.get('dec-hidden-size'))
    ...
    def forward(self, dec_hidden, enc_hiddens, enc_padding_mask, enc_temporal_score):
        ...
        score = self.v(t.tanh(dec_out + enc_out)).squeeze(2)    # B, L
